-- Fix security vulnerability in usuarios table
-- Add role-based access control and implement proper RLS policies

-- Step 1: Add role column to usuarios table
ALTER TABLE public.usuarios ADD COLUMN IF NOT EXISTS role TEXT NOT NULL DEFAULT 'user';

-- Step 2: Create role enum for better data integrity
CREATE TYPE public.user_role AS ENUM ('admin', 'user');

-- Step 3: Update the role column to use the enum
ALTER TABLE public.usuarios ALTER COLUMN role TYPE user_role USING role::user_role;
...
-- Step 6: Create function to get current user ID from usuarios table
...
-- Step 9: Set the first user as admin (if there are users without roles)
-- This ensures there's always at least one admin
DO $$
BEGIN
  -- Make the first user an admin if no admin exists
  IF NOT EXISTS (SELECT 1 FROM public.usuarios WHERE role = 'admin') THEN
