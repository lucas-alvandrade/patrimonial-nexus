-- Fix security vulnerability in usuarios table (final corrected version)
...
-- Only admins can insert new users (this policy will be updated for LDAP auth)
...
-- Step 8: Set the first user as admin (if there are users without admin role)
-- This ensures there's always at least one admin
DO $$
BEGIN
  -- Make the first user an admin if no admin exists and there are users
